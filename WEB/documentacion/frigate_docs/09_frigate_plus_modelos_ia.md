# Modelos de IA y Entrenamiento Personalizado

*Documentación Técnica Oficial Benthic Core — Módulo 9*
*Fuente Original:* [https://docs.frigate.video/plus/](https://docs.frigate.video/plus/)

> **Navegación Rápida:** ◀️ **Anterior:** [Zonas y Máscaras de Detección](./08_zonas_y_mascaras.md) | ➡️ **Siguiente:** [Integración con Home Assistant y MQTT](./10_integracion_home_assistant_mqtt.md)
...
Model TypeDescription`mobiledet`Based on the same architecture as the default model included with Frigate. Runs on Google Coral devices and CPUs.`yolonas`A newer architecture that offers slightly higher accuracy and improved detection of small objects. Runs on Intel, NVidia GPUs, and AMD GPUs.`yolov9`A leading SOTA (state of the art) object detection model with similar performance to yolonas, but on a wider range of hardware options. Runs on most hardware.
...
infoWhen switching to YOLOv9, you may need to adjust your thresholds for some objects.

#### Hailo Support[​](#hailo-support)
...
Rockchip models are automatically converted as of 0.17. For 0.16, YOLOv9 onnx models will need to be manually converted. First, you will need to configure Frigate to use the model id for your YOLOv9 onnx model so it downloads the model to your `model_cache` directory. From there, you can follow the [documentation](/configuration/object_detectors#converting-your-own-onnx-model-to-rknn-format) to convert it.
...
HardwareRecommended Detector TypeRecommended Model Type[CPU](/configuration/object_detectors#cpu-detector-not-recommended)`cpu``mobiledet`[Coral (all form factors)](/configuration/object_detectors#edge-tpu-detector)`edgetpu``yolov9`[Intel](/configuration/object_detectors#openvino-detector)`openvino``yolov9`[NVidia GPU](/configuration/object_detectors#onnx)`onnx``yolov9`[AMD ROCm GPU](/configuration/object_detectors#amdrocm-gpu-detector)`onnx``yolov9`[Hailo8/Hailo8L/Hailo8R](/configuration/object_detectors#hailo-8)`hailo8l``yolov9`[Rockchip NPU](/configuration/object_detectors#rockchip-platform)`rknn``yolov9`
...
![Imagen](/assets/images/attribute-example-face-bb4c144f9027abc0572237a2d04a2943.jpg)

Delivery logos such as `amazon`, `ups`, and `fedex` labels are used to automatically assign a sub label to car objects.

![Imagen](/assets/images/attribute-example-fedex-2611458c9a8a703362d7ea79d4701f3b.jpg)

[Edit this page](https://github.com/blakeblackshear/frigate/edit/master/docs/docs/plus/index.md)
...
| :--- | ---: |
| ◀️ **Anterior:** [Zonas y Máscaras de Detección](./08_zonas_y_mascaras.md) | ➡️ **Siguiente:** [Integración con Home Assistant y MQTT](./10_integracion_home_assistant_mqtt.md) |
